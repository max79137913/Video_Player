                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine, "%s %f %f %f %d %d", &ImagePath, &scaleWidth, &scaleHeight ,&fps, &anti,&aspect);
	inImage.setWidth(w);
	inImage.setHeight(h);
	inImage.setoldWidth(w);
	inImage.setoldHeight(h);
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	int FrameCounter=0;
	int Maxframenum=outImage.numofFrame();
	RECT rt;


	outImage.SetupVideo();

	if(aspect==1){	outImage.AspectRatio(scaleWidth,scaleHeight);}

	if(aspect==0){outImage.Rescale(scaleWidth,scaleHeight); } //after rescale width, height is new
	if(anti==1){outImage.AntiAlias();}


	while(FrameCounter < Maxframenum){
		

		outImage.DisplayVideo(FrameCounter);
